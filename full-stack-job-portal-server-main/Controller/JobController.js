...
module.exports.updateSingleJob = async (req, res, next) => {
    ...
    try {
        ...
        if (!isJobExists) {
            ...
            // Check job ownership
            if (isJobExists.createdBy.toString() !== req.user._id.toString()) {
                return next(createError(403, "You are not authorized to edit this job. Only the job creator can edit it."));
            }
...
module.exports.deleteSingleJob = async (req, res, next) => {
    ...
    try {
        ...
        if (!isJobExists) {
            ...
            // Check job ownership
            if (isJobExists.createdBy.toString() !== req.user._id.toString()) {
                return next(createError(403, "You are not authorized to delete this job. Only the job creator can delete it."));
            }
...
module.exports.updateJobStatus = async (req, res, next) => {
    ...
    try {
        ...
        // Verify that the user is the job creator (recruiter)
        if (job.createdBy.toString() !== req.user._id.toString()) {
            return next(createError(403, "Unauthorized to update this job status"));
        }
        ...
        // If job is marked as DECLINED, auto-reject all pending applications
