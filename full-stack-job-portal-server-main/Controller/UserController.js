...
exports.addUser = async (req, res, next) => {
    ...
    try {
        ...
        if (isUserExists) {
            ...
            // If user selected admin role, validate secret code
            else if (data.role === "admin") {
                if (data.adminCode === ADMIN_SECRET_CODE) {
                    assignedRole = "admin";
                } else {
                    return next(createError(401, "Invalid admin code. Admin registration denied."));
                }
            }
            // If user selected recruiter role, assign it
            else if (data.role === "recruiter") {
                assignedRole = "recruiter";
            }
            // Otherwise default to user
            
            req.body.role = assignedRole;
